...
all_sectors <- ggplot(carbon, aes(x=abatement)) +
  ...
  geom_smooth(aes(y=mc_b), method = lm, se = TRUE, size = 0.5, color = "blue") +
  geom_smooth(aes(y=mc_c), method = lm, se = TRUE, size = 0.5, color = "green") +
...
kink1_q <- (int_a - int_c)/(slope_c - slope_a)
...
costa_tax <- integrate(abate_a, lower = 0, upper = qtax_a)$value

# Sector B 
qtax_b <- qb_abate(optimal_tax) #abates 151.5
costb_tax <- integrate(abate_b, lower = 0, upper = qtax_b)$value

# Sector C 
qtax_c <- qc_abate(optimal_tax) # abates 65.2
costc_tax <- integrate(abate_c, lower = 0, upper = qtax_c)$value

# Aggregated cost
costagg_tax <- integrate(agg_abate, lower = 0, upper = 300)$value
...
The total cost of the carbon tax is the area under the aggregate cost curve from 0 to 300.  
...
$Total~Cost~Cap~and~Trade = `r round(costagg_tax, digits=2)`$  
...
# Find the intersection of the demand curve for Sector D and the aggregate abatement curve for country x 

q_intersect <- (int_lineard - (agg_intq_all/agg_slopeq_all))/((1/agg_slopeq_all) - slope_d)
```
...
From the previous question, if country Y enters the carbon market, Country X will be emitting, so the amount of air pollution will be ...

Since the Country Y will be abating XX more, they will be emitting XX tons of carbon, and that will result in XX tons of air pollution.
